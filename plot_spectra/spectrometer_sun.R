...
gt::gt(sol, caption = "Porcentagem das bandas R, G, B, PhR no PPFD") |>
    gt::fmt_number(
        columns = c(Red, Green, Blue, PhR),
        decimals = 3
    ) |>
    gt::tab_footnote(
        footnote = "PhR: far-red",
        locations = gt::cells_column_labels(columns = PhR)
    ) |>
    gt::tab_footnote(
        footnote = "Hora: horário de coleta",
        locations = gt::cells_column_labels(columns = Hora)
    )
